...
class PIHDataRandom(Dataset):
    ...
    def __getitem__(self, index):
        ...
        imag_torch = T.functional.adjust_contrast(
            ground_truth, (np.random.rand() * 0.3 + 0.85)
        )

        imag_torch = T.functional.adjust_brightness(
            imag_torch, (np.random.rand() * 0.3 + 0.85)
        )
        ...
        imag_torch[0, ...] = imag_torch[0, ...] * (np.random.rand() * 0.2 + 0.80)

        imag_torch[1, ...] = imag_torch[1, ...] * (np.random.rand() * 0.2 + 0.80)

        imag_torch[2, ...] = imag_torch[2, ...] * (np.random.rand() * 0.2 + 0.80)
